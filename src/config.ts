...
export const config = {
  ...
  poolAccountId: process.env.ACCOUNT_ID || fail("ACCOUNT_ID not set"),
  transactionFeeStroops: Number.parseInt(process.env.TX_FEE_STROOPS || fail("TX_FEE_STROOPS not set"), 10),
  /** in seconds */
  transactionValidity: process.env.TX_VALIDITY_SECS ? Number.parseFloat(process.env.TX_VALIDITY_SECS) : 15
}
